...
```{r Runtime vs Revenue}
ggplot(data)+
  geom_point(aes(x = runtime, y = revenue),color = "dark green", alpha = 0.5)
```
Movies that have higher revenue are not necessarily the longest movies or vice versa. The scatter plot shows no noticable relationship between the runtime of the movies and it's revenue.

```{r runtimes and budget}
ggplot(data = data)+
  geom_point(aes(x = runtime, y = budget), color = "dark green", alpha = 0.5)


```
We assumed that movies with longer runtimes might have bigger budgets. But from the scatter plot above, we can see that it is not the case. The movies that have longer runtimes doesn't necessaritly have the bigger budget than those of shorter movies.

```{r runtime and languages}
...
```
Russian movies have the lowest runtimes while Marathi (One of the Indian languages) has the highest runtime. Followed by Tamil (Also an Indian languge). Fourth longest is Telugu and fifth is Hindi (Again, both are Indian languages). Indonesia has the third longest runtime and Vietnamese comes in at sixth. Overall, south east Asian countries seem to have the longest movies.
...
```{r Popular genres}
genres_df$names.factored <- as.factor(genres_df$name)
summary.genres <- genres_df %>% group_by(names.factored) %>% summarize(num1 = n())

ggplot(data = summary.genres) +
  geom_bar(aes(x = reorder(names.factored, num1), y = num1), fill = "dark green", alpha = 0.5, stat = "identity") +
  coord_flip()

## Sorted now, just have to turn the columns into factors, and then reorder() them by the count of that factor.
...
#This is a tougher one, but here's a way to make it work...

# data has the column we need called "id", but genres_df calls the same thing "movie_id" and uses the "id" column for genre id.  We need to fix that by adding a column to genre called "movie_id"

data$movie_id <- data$id

# The genres_df has movie_id as a character, which we need to make a numeric to match the original data tibble we have.

genres_df$movie_id <- as.numeric(genres_df$movie_id)

genre.and.popularity <- left_join(data, genres_df, by = "movie_id")
#print(head(genre.and.popularity))

popularity.average <- genre.and.popularity %>% group_by(names.factored) %>% summarise(avg.pop = mean(popularity))

ggplot(data = popularity.average) +
  geom_bar(aes(x = reorder(names.factored, avg.pop), y = avg.pop), fill = "dark green", alpha = 0.5, stat = "identity") +
  coord_flip()

# check out https://www.rdocumentation.org/packages/dplyr/versions/0.7.8/topics/summarise for more on the super useful summarise() function in dplyr.

```

Fantasy, Adventure, and Science fiction are the most popular (on average), but I would assume they are also more expensive to produce.

```{r Popular production companies}

total.names <- table(production_companies_df$name)

names.important <- subset(production_companies_df, name %in% names(total.names[total.names > 20]))

ggplot(data = names.important)+
...
##As an option, as you can see I restricted the plot to companies that have more than 20 entries in the database.
```


```{r language and revenue}
ggplot(data)+
...
gender.count <- select(cast_df, one_of(c("gender")))
gender.count <- filter(gender.count, gender == 1 | gender == 2)

ggplot(gender.count, aes(x =gender, ..count..)) +     
  geom_bar()

## Unable to print plot properly. Don't know what's going wrong..!
#names(data)

## This one took me a while to figure out, it's a data cleaning problem. Many of the genders are 1 for female, 2 for male, and 0 for unknown or unimportant (I suppose), but in other rows it's a person's name. When it is factored, all of those names end up as factors. By selecting only the correct rows, we get what I think you were looking for.


```

```{r Popularity vs Revenue}
ggplot(data)+
  geom_point(aes(x = revenue, y = popularity), color = "dark green", alpha = 0.5)

```

One would assume that a popoular movie would bring in more revuenue. This seems to be not the case. From the popularity vs revenue scatter plot above, a popular movie doesn't necessarily bring in more revenue. It can also be said that a movie that has good amount of boxoffice collections might not be popular. Let's look at the correlation between movie popularity and it's revenue.

```{r correlation of popularity and revenue}
cor(data$revenue, data$popularity)
```

The correlation value of 0.46 suppors our findings from the popularity vs revenue scatter plot. It shows a weak relationship between the two variables. We can assume that some movies are popular but make less money because their target country has less money to spend and tickets are cheaper, but it's also possible that people liked the movie after it left the box office and so the movie made less money.

```{r language and revenue2}
ggplot(data)+
  geom_boxplot(aes(x = fct_reorder(original_language, revenue, .desc = T), y = revenue), color = "dark blue")+
  xlab("Original language")
```
Unsuprisingly, english movies has the highest revenue among all the languages. English movies overwhelmengly had the highest budget and highest revenue.Japanese movies which has the second highest revenue is among the movies with high budget. This box plot combined with the budget vs original language boxplot actually shows the relationship depected by the budget vs revenue scatter plot. There is a clear relationship between the budget and the revenue. Let's look at the correlation.

```{r correlation of budget vs revenue}
cor(data$budget, data$revenue)
```

Correlation of 0.75 is fairly high and a good indicator of relationship between revenue and budget.

```{r popular languages}
ggplot(data)+
  geom_boxplot(aes(x = fct_reorder(original_language, popularity, .desc = T), y = popularity), color = "dark blue")+
  xlab("Original language")
```
To our suprise, Norwegian language seems to be the most popular followed by Indonesian and then english. Hebrew comes in at fourth and japanese at fifth. Indian movies, tough they are lengthy, are not liked by everyone. Perhaps the Norweigians and Indonesians make so few movies, that everything they make is popular in that country?

```{r number of movies for each language in dataset}
table(data$original_language)
```

This suggests our hypothesis above may be correct, only two Nowegian movies, and one Indonesian movies. People in those countries got so excited about those three movies, their popularity is high.

```{r visualize the table above}
ggplot(data)+
  geom_bar(aes(original_language), fill = "dark green", alpha = 0.5)
```
...
```{r raw profit}
data <- mutate(data, raw.profit = revenue - budget)

plot <- ggplot(data, aes(raw.profit)) + geom_histogram()
print(plot)

```

It would appear most movies make a little, some lose money, and very few make huge profits.

Let's see profit as a function of percentage return.

```{r percent profit}
data <- mutate(data, percent.profit = revenue / budget)
# we have to leave out rows with missing/obviously wrong data
useful.rows <- data[data$budget > 10000, ]

#plot <- ggplot(data, aes(percent.profit)) + 
#  geom_histogram(binwidth = 100, aes(y = ..density..))
#print(plot)

#how many movies were profitable?
print(count(useful.rows[useful.rows$percent.profit > 0, ]))

#2165

#how many movies made double their money?
print(count(useful.rows[useful.rows$percent.profit > 2, ]))
#1081
```

So, for the 3,000 observations, 2,165 made some kind of profit, and 1,081 made double their money back (revenue was two times their budget)

What was the most profitable?
```{r max percent profit}
print(max(useful.rows$percent.profit))
print(useful.rows$original_title[useful.rows$percent.profit > 12000])
```

Paranormal Activity is the most profitable movie in our database, which is confirmed by the wikipedia article on this film. (https://en.wikipedia.org/wiki/Paranormal_Activity)

It cost $15,000 to make, and made 193.4 million in revenue.  That's a 12,000% profit! 
